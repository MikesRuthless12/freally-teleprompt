...
import-not-itemised = Bir PDF'in içeriği listelenemez - metni orijinaliyle karşılaştırın.
...
## Kısayollar, ayak pedalları ve genel kısayollar (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = Kısayollar ve pedallar
shortcuts-intro = Değiştirmek istediğiniz atamaya basın, ardından kullanmak istediğiniz tuşa, kumanda düğmesine veya pedala basın. «Uygulamada» kısayolları Freally Teleprompt öndeyken çalışır; «Her yerde» olanlar nerede olursanız olun çalışır.
shortcuts-search = Komut ve tuş ara
shortcuts-command = Komut
shortcuts-in-app = Uygulamada
shortcuts-global = Her yerde
shortcuts-window-only = Yalnızca uygulamada
shortcuts-no-matches = Eşleşen komut yok.
shortcuts-listening = { $command } için bir tuşa basın, iptal için Esc
shortcuts-listening-short = Bir tuşa basın…
shortcuts-rebind = { $command } atamasını değiştir
shortcuts-clear = { $command } atamasını temizle
shortcuts-conflict = Şuna da atanmış: { $others }
shortcuts-not-registered = Bu tuşu başka bir program kullanıyor ({ $reason })
shortcuts-wayland = Wayland, uygulamaların tuşları sistem genelinde ayırmasına izin vermez; bu nedenle «Her yerde» atamaları bu oturumda çalışmayabilir.
shortcuts-reset = Varsayılanlara dön
shortcuts-cancel = İptal
shortcuts-apply = Uygula
cmd-play-pause = Oynat / duraklat
cmd-stop = Durdur
cmd-top = Başa dön
cmd-faster = Hızlandır
cmd-slower = Yavaşlat
cmd-step-back = Bir adım geri
cmd-step-forward = Bir adım ileri
cmd-next-marker = Sonraki bölüm
cmd-prev-marker = Önceki bölüm
cmd-find = Bul ve değiştir
