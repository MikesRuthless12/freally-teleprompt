...
import-not-itemised = Zawartości pliku PDF nie da się wyliczyć - porównaj tekst z oryginałem.
...
## Skróty, pedały i skróty globalne (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = Skróty i pedały
shortcuts-intro = Kliknij przypisanie, aby je zmienić, a następnie naciśnij wybrany klawisz, przycisk pilota lub pedału. Skróty „W aplikacji” działają, gdy Freally Teleprompt jest na pierwszym planie; skróty „Wszędzie” działają niezależnie od tego, gdzie pracujesz.
shortcuts-search = Szukaj poleceń i klawiszy
shortcuts-command = Polecenie
shortcuts-in-app = W aplikacji
shortcuts-global = Wszędzie
shortcuts-window-only = Tylko w aplikacji
shortcuts-no-matches = Żadne polecenie nie pasuje.
shortcuts-listening = Naciśnij klawisz dla { $command } albo Esc, aby anulować
shortcuts-listening-short = Naciśnij klawisz…
shortcuts-rebind = Zmień przypisanie dla { $command }
shortcuts-clear = Usuń przypisanie dla { $command }
shortcuts-conflict = Przypisane także do { $others }
shortcuts-not-registered = Inny program używa tego klawisza ({ $reason })
shortcuts-wayland = Wayland nie pozwala aplikacjom rezerwować klawiszy w całym systemie, więc przypisania „Wszędzie” mogą nie działać w tej sesji.
shortcuts-reset = Przywróć domyślne
shortcuts-cancel = Anuluj
shortcuts-apply = Zastosuj
cmd-play-pause = Odtwórz / wstrzymaj
cmd-stop = Zatrzymaj
cmd-top = Powrót na początek
cmd-faster = Przyspiesz
cmd-slower = Zwolnij
cmd-step-back = Krok wstecz
cmd-step-forward = Krok naprzód
cmd-next-marker = Następna sekcja
cmd-prev-marker = Poprzednia sekcja
