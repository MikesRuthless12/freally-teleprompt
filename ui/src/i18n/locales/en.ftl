...
import-not-itemised = A PDF's contents cannot be listed - compare the text with the original.
...
## Shortcuts, pedals and global hotkeys (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = Shortcuts and pedals
shortcuts-intro = Press a binding to change it, then press the key, remote or pedal button you want. In-app keys work while Freally Teleprompt is in front; global ones work wherever you are.
shortcuts-search = Search commands and keys
shortcuts-command = Command
shortcuts-in-app = In the app
shortcuts-global = Anywhere
shortcuts-window-only = In the app only
shortcuts-no-matches = No commands match that.
shortcuts-listening = Press a key for { $command }, or Escape to cancel
shortcuts-listening-short = Press a key…
shortcuts-rebind = Change the binding for { $command }
shortcuts-clear = Clear the binding for { $command }
shortcuts-conflict = Also bound to { $others }
shortcuts-not-registered = Another program is using this key ({ $reason })
shortcuts-wayland = Wayland does not let applications claim keys system-wide, so "Anywhere" bindings may not work in this session.
shortcuts-reset = Restore defaults
shortcuts-cancel = Cancel
shortcuts-apply = Apply
cmd-play-pause = Play / pause
cmd-stop = Stop
cmd-top = Back to the top
cmd-faster = Speed up
cmd-slower = Slow down
cmd-step-back = Step back
cmd-step-forward = Step forward
cmd-next-marker = Next section
cmd-prev-marker = Previous section
