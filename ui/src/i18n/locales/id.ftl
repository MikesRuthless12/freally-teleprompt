...
import-not-itemised = Isi PDF tidak dapat dirinci - bandingkan teksnya dengan aslinya.
...
## Pintasan, pedal kaki, dan pintasan global (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = Pintasan dan pedal
shortcuts-intro = Klik sebuah penetapan untuk mengubahnya, lalu tekan tombol, remote, atau pedal yang Anda inginkan. Pintasan «Di dalam aplikasi» bekerja selama Freally Teleprompt berada di depan; yang «Di mana saja» bekerja di mana pun Anda berada.
shortcuts-search = Cari perintah dan tombol
shortcuts-command = Perintah
shortcuts-in-app = Di dalam aplikasi
shortcuts-global = Di mana saja
shortcuts-window-only = Hanya di dalam aplikasi
shortcuts-no-matches = Tidak ada perintah yang cocok.
shortcuts-listening = Tekan tombol untuk { $command }, atau Esc untuk membatalkan
shortcuts-listening-short = Tekan sebuah tombol…
shortcuts-rebind = Ubah penetapan untuk { $command }
shortcuts-clear = Hapus penetapan untuk { $command }
shortcuts-conflict = Juga ditetapkan ke { $others }
shortcuts-not-registered = Program lain sedang memakai tombol ini ({ $reason })
shortcuts-wayland = Wayland tidak mengizinkan aplikasi memesan tombol untuk seluruh sistem, sehingga penetapan «Di mana saja» mungkin tidak bekerja pada sesi ini.
shortcuts-reset = Kembalikan ke bawaan
shortcuts-cancel = Batal
shortcuts-apply = Terapkan
cmd-play-pause = Putar / jeda
cmd-stop = Hentikan
cmd-top = Kembali ke awal
cmd-faster = Percepat
cmd-slower = Perlambat
cmd-step-back = Mundur satu langkah
cmd-step-forward = Maju satu langkah
cmd-next-marker = Bagian berikutnya
cmd-prev-marker = Bagian sebelumnya
cmd-find = Cari dan ganti
