...
import-not-itemised = PDF의 내용은 항목별로 알 수 없습니다. 원본과 텍스트를 비교해 보세요.
...
## 단축키, 풋 페달, 전역 단축키 (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = 단축키와 페달
shortcuts-intro = 바꾸려는 할당을 누른 다음 원하는 키, 리모컨 버튼 또는 페달을 누르세요. '앱 안에서' 단축키는 Freally Teleprompt가 앞에 있을 때 작동하고, '어디서나' 단축키는 어디에 있든 작동합니다.
shortcuts-search = 명령과 키 검색
shortcuts-command = 명령
shortcuts-in-app = 앱 안에서
shortcuts-global = 어디서나
shortcuts-window-only = 앱 안에서만
shortcuts-no-matches = 일치하는 명령이 없습니다.
shortcuts-listening = { $command }에 사용할 키를 누르세요. 취소하려면 Esc
shortcuts-listening-short = 키를 누르세요…
shortcuts-rebind = { $command } 할당 변경
shortcuts-clear = { $command } 할당 지우기
shortcuts-conflict = { $others }에도 할당됨
shortcuts-not-registered = 다른 프로그램이 이 키를 사용 중입니다 ({ $reason })
shortcuts-wayland = Wayland에서는 앱이 시스템 전체 키를 차지할 수 없으므로 이 세션에서는 '어디서나' 할당이 작동하지 않을 수 있습니다.
shortcuts-reset = 기본값 복원
shortcuts-cancel = 취소
shortcuts-apply = 적용
cmd-play-pause = 재생 / 일시정지
cmd-stop = 정지
cmd-top = 처음으로
cmd-faster = 빠르게
cmd-slower = 느리게
cmd-step-back = 뒤로 한 단계
cmd-step-forward = 앞으로 한 단계
cmd-next-marker = 다음 구간
cmd-prev-marker = 이전 구간
cmd-find = 찾아 바꾸기
