...
import-not-itemised = El contenido de un PDF no se puede enumerar: compare el texto con el original.
...
## Atajos, pedales y atajos globales (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = Atajos y pedales
shortcuts-intro = Pulse una asignación para cambiarla y luego presione la tecla, el mando o el pedal que quiera. Los atajos «En la aplicación» funcionan mientras Freally Teleprompt está en primer plano; los de «En cualquier parte» funcionan esté donde esté.
shortcuts-search = Buscar comandos y teclas
shortcuts-command = Comando
shortcuts-in-app = En la aplicación
shortcuts-global = En cualquier parte
shortcuts-window-only = Solo en la aplicación
shortcuts-no-matches = Ningún comando coincide.
shortcuts-listening = Pulse una tecla para { $command }, o Esc para cancelar
shortcuts-listening-short = Pulse una tecla…
shortcuts-rebind = Cambiar la asignación de { $command }
shortcuts-clear = Borrar la asignación de { $command }
shortcuts-conflict = También asignada a { $others }
shortcuts-not-registered = Otro programa está usando esta tecla ({ $reason })
shortcuts-wayland = Wayland no permite que las aplicaciones reserven teclas para todo el sistema, por lo que las asignaciones «En cualquier parte» pueden no funcionar en esta sesión.
shortcuts-reset = Restaurar valores predeterminados
shortcuts-cancel = Cancelar
shortcuts-apply = Aplicar
cmd-play-pause = Reproducir / pausar
cmd-stop = Detener
cmd-top = Volver al principio
cmd-faster = Acelerar
cmd-slower = Ralentizar
cmd-step-back = Retroceder un paso
cmd-step-forward = Avanzar un paso
cmd-next-marker = Sección siguiente
cmd-prev-marker = Sección anterior
