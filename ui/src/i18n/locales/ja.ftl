...
import-not-itemised = PDF の内容は一覧にできません。元の文書とテキストを見比べてください。
...
## ショートカット・フットペダル・グローバルホットキー (FT-M04 / FT-M13 / FT-M16)
shortcuts-title = ショートカットとペダル
shortcuts-intro = 変更したい割り当てを押してから、使いたいキー・リモコン・ペダルのボタンを押してください。「アプリ内」のキーは Freally Teleprompt が手前にあるときに動作し、「どこでも」のキーはどの画面にいても動作します。
shortcuts-search = コマンドとキーを検索
shortcuts-command = コマンド
shortcuts-in-app = アプリ内
shortcuts-global = どこでも
shortcuts-window-only = アプリ内のみ
shortcuts-no-matches = 一致するコマンドはありません。
shortcuts-listening = { $command } に割り当てるキーを押してください（Esc で取り消し）
shortcuts-listening-short = キーを押してください…
shortcuts-rebind = { $command } の割り当てを変更
shortcuts-clear = { $command } の割り当てを消去
shortcuts-conflict = { $others } にも割り当て済み
shortcuts-not-registered = このキーは別のプログラムが使用しています（{ $reason }）
shortcuts-wayland = Wayland ではアプリがシステム全体のキーを確保できないため、このセッションでは「どこでも」の割り当てが動作しない場合があります。
shortcuts-reset = 既定に戻す
shortcuts-cancel = キャンセル
shortcuts-apply = 適用
cmd-play-pause = 再生 / 一時停止
cmd-stop = 停止
cmd-top = 先頭に戻る
cmd-faster = 速く
cmd-slower = 遅く
cmd-step-back = 少し戻る
cmd-step-forward = 少し進む
cmd-next-marker = 次のセクション
cmd-prev-marker = 前のセクション
cmd-find = 検索と置換
